...
static void *search4TriBasesThread(void *data) {
    quad_threadinfo_t info = *(quad_threadinfo_t *) data;
    ...
    while (seed < end) {
        if (isTriBase(info.sconf, seed, info.quality)) {
            fprintf(fp, "%"PRId64"\n", seed);
            fflush(fp);
            //printf("Thread %d: %"PRId64"\n", info.threadID, seed);
        }
...
void search4TriBases(const char *fnam, const int threads,
    ...
    for (t = 0; t < threads; t++) {
        pthread_create(&threadID[t], NULL, search4TriBasesThread, (void *) &info[t]);
    }
...
Pos getSpawn(const int mcversion, LayerStack *g, int *cache, int64_t worldSeed, int nbr) {
